     ...
     <form name="dealform" action="dealservlet" method="post">
      ...
      <%
        PreparedStatement pr = null;
  	    ...
  	    String sql = "select stime,etime from exam where id=?";
  	    pr  = (PreparedStatement) conn.prepareStatement(sql);
  	    pr.setInt(1, Integer.parseInt(id));
  	    re = pr.executeQuery();
  	    re.next();
  	    String stime = re.getString(1);
  	    String etime = re.getString(2);
        SimpleDateFormat sim=new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
  	    Date d1 = sim.parse(stime);
        Date d2 = sim.parse(etime);
  	    long diff = (d2.getTime() - d1.getTime())/1000;
  		sql="select q.qid,q.equation,q.inputans,q.correctans from que q where q.id = ?";
  		pr  = (PreparedStatement) conn.prepareStatement(sql);
  		pr.setString(1, id);
  	    re = pr.executeQuery();
